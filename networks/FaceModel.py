...
def get_model(url, net, embedding_size, no_gpu=False):
    ...
    model_path = "ckpts/{}".format(model_name)
    try:
        print(f"Load existing checkpoint: {model_path}")
        ...
        print(f"Load checkpoint: {os.path.join(os.getcwd(), model_path)}")
        checkpoint = torch.load(
            model_path,
            map_location=lambda storage, loc: storage.cuda(),
        )
...
class FaceModel:
    def __init__(self, url, net, **kwargs):
        ...
        output, input_device = get_model(
            net=net, url=url, embedding_size=embedding_size, no_gpu=no_gpu
        )
